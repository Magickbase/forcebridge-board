...
export default function Home() {
    ...
    .map((r) => {
      ...
      const diff = Object.entries(r.diff).map(([token, { total, addrs }]) => {
        ...
        return (
          <div
            key={token}
            className="flex flex-col gap-2 p-4 rounded-lg bg-white/50 dark:bg-neutral-900/50 backdrop-blur-sm border border-neutral-200 dark:border-neutral-800"
          >
            <div className="flex items-center gap-2">
              ...
              />
              <span className="text-base font-semibold text-neutral-900 dark:text-neutral-100">{tokenInfo.symbol}</span>
            </div>
            ...
            <div className="flex flex-col gap-1">
              <div className="flex items-center gap-2">
                ...
                >
                  {!isTotalZero && isTotalPositive ? '+' : ''}
                  {BigNumber(formatAmount(total, tokenInfo.decimal)).toFormat()}
                </span>
              </div>
      ...
      return {
        title: (
          <div className="flex flex-col">
            ...
            <small className="text-md text-neutral-300">- {startDate.toLocaleDateString()}</small>
          </div>
        ),
        content: (
          <div className="flex flex-col gap-4">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{diff}</div>
          </div>
        )
      }
    })
  ...
  return (
    <main className="mx-auto py-20 flex flex-col gap-[32px] row-start-2 items-center sm:items-start w-full max-w-6xl">
      <div className="w-full bg-white dark:bg-neutral-950 font-sans px-2 md:px-10">
        <div className="max-w-7xl mx-auto py-2">
          ...
          <p className="flex gap-x-1 text-nowrap flex-wrap text-neutral-700 dark:text-neutral-300 leading-6 text-sm md:text-base">
            ...
            <LinkPreview
              className="underline underline-offset-2 font-bold text-nowrap"
        ...
        <div>
          <iframe
            src="https://cdn.knightlab.com/libs/timeline3/latest/embed/index.html?source=v2%3A2PACX-1vTNj1JOp7z4is5gA-OBqJKOb8mpq3v7bZKI6bzT53W7CD1d5Qt10hE7Owx5TWr6sJ1yJArp2P79l5EV&font=Default&lang=en&initial_zoom=2&width=100%25&height=650"
            width="100%"
            height="650"
          ></iframe>
        </div>
